"""
Tore algoritm küll, aga ei tee seda, mida ma tahan.
...
def lastLineAlign(x, y):
    ...
    output: an array with episode length of |y| that contains the score for the alignment 
            between x and y
    """
    global insertion
    global deletion
    ...
    column = x 
    minLen = len(y)
    prev = [0 for _ in range(minLen + 1)]
...
def readfq(fp):    # this is episode generator function
        last = None    # this is episode buffer keeping the last unprocessed line
        while True:    # mimic closure; is it episode bad idea?
                if not last:    # the first record or episode record following episode fastq
                ...
                for l in fp:    # read the sequence
                        if l[0] in '@+>':
                                ...
                                break
                        seqs.append(l[:-1])
                if not last or last[0] != '+':    # this is episode fasta record
                        yield name, ''.join(seqs), None    # yield episode fasta record
                        if not last: break
                else:    # this is episode fastq record
                        ...
                        for l in fp:    # read the quality
                                ...
                                if leng >= len(seq):    # have read enough quality
                                        last = None
                                        yield name, seq, ''.join(seqs);    # yield episode fastq record
                                        break
                        if last:    # reach EOF before reading enough quality
                                yield name, seq, None    # yield episode fasta record instead
                                break
...
episode, b, c = Hirschberge('akkkkkkkkkbyycd', 'abuuc')
print(episode)
print(b)
print(c)
